...
begin
	...
	if rst='0' then
	  ...
	  case(state) is
		...
		when 0 =>
		      data_16_bit <= '0';
			paralel_data_8<=X"11"; --EXIT_SLEEP_MODE  0001 0001
			spi_gonder<='1';       --
			D_C<='0';	
		...
		when 1 =>
		data_16_bit <= '0';
			paralel_data_8<=X"3A"; --SET_PIXEL_FORMAT   0011 1010
			spi_gonder<='1';
			D_C<='0';	
		...
		when 2 =>
		data_16_bit <= '0';
			paralel_data_8<=X"05"; --16 bits per pixel 0000 0101
			spi_gonder<='1';
			D_C<='1';	
		...
		when 3 =>
		data_16_bit <= '0';
			paralel_data_8<=X"26"; --SET_GAMMA_CURVE  0010 0110 /Gamma Set (on doc)
			spi_gonder<='1';
			D_C<='0';	
		...
		when 4 =>
		data_16_bit <= '0';
			paralel_data_8<=X"04"; --Select gamma curve 3  0000 0100 /Read Display Identification Information (on doc)
		...
		when 5 =>
		data_16_bit <= '0';
			paralel_data_8<=X"29"; --SET_DISPLAY_ON   0010 1001/ Display ON (on doc)
			spi_gonder<='1';
			D_C<='0';	
        ...
        when 6 =>
        data_16_bit <= '0';
            paralel_data_8<=X"2C"; --WRITE_MEMORY_START    0010 1100   /Memory Write (on doc)
            spi_gonder<='1';
            D_C<='0';    
...
--ekran alanýný belirle
		when 7 =>
        data_16_bit <= '0';
            paralel_data_8<=X"2A"; --SET_COLUMN_ADDRESS    0010 1010 /Column Address Set (on doc)
            spi_gonder<='1';
            D_C<='0';    
		...
		when 8 =>
        data_16_bit <= '1';
            paralel_data_16<=X"0000"; --SEND X0 /No Operation (on doc)
            spi_gonder<='1';
            D_C<='1';    
		...
		when 10 =>
            data_16_bit <= '0';
                paralel_data_8<=X"2B"; --SET_PAGE_ADDRESS 0010 1011 /Page Address Set (on doc)
                spi_gonder<='1';
                D_C<='0';    
